...
export default function HomeworkDrawer({
  ...
  const remove = async (id: string) => {
    const supabase = getSupabase();
    if (!supabase) return;
    if (!confirm("Delete this homework and all submissions?")) return;
                  ...
                  <div className="mt-3 flex items-center justify-between gap-2">
                    {!isHost ? (
                      <button
                        ...
                        className="text-xs rounded-md border border-brand-500/40 text-brand-200 hover:bg-brand-500/10 px-2.5 py-1"
                      >
                        ...
                        <li key={s.id} className="flex items-center gap-2 text-xs">
                          <span className="flex-1 min-w-0">
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="ml-2 text-brand-300 hover:underline truncate"
                            >
                              {s.file_name}
                            </a>
                          </span>
