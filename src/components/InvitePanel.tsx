    ...
    (async () => {
      try {
        ...
        const svg = new QR({
          ...
          width: 240,
          height: 240,
          // --text's literal value: the QR is rendered as standalone SVG
          // markup so it can't read CSS vars; still near-black for scanners.
          color: "#1C1B19",
          background: "#ffffff",
          ecl: "M",
          join: true,
    ...
    >
      ...
      >
        <header className="flex items-center justify-between px-5 py-4 border-b-2 border-dashed border-[color:var(--border)]">
          <h2 className="text-lg font-extrabold tracking-display">Invite</h2>
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-full bg-[var(--bg-elev)] border-2 border-ink shadow-sticker-sm sticker-press text-[var(--text)] hover:bg-[var(--bg-elev-2)] inline-flex items-center justify-center"
            aria-label="Close"
          >
        ...
        <div className="p-5 space-y-4">
          {/* bg-white on purpose (not --bg-elev): the QR needs a true white
              quiet zone for phone cameras, whatever the surface token is. */}
          <div className="rounded-xl border-2 border-ink p-3 bg-white flex items-center justify-center">
            {qrSvg ? (
              <div
              ...
              <div className="w-[240px] h-[240px] flex items-center justify-center text-sm font-semibold text-[var(--text-muted)]">
                Generating…
              </div>
            )}
          </div>
          ...
          <div className="flex gap-2">
            <input
              readOnly
              value={inviteUrl}
              className="flex-1 min-w-0 rounded-lg bg-[var(--bg-elev)] border-2 border-ink shadow-sticker-sm px-3.5 py-2 text-sm font-semibold text-[var(--text)] outline-none focus:border-brand-600 focus:shadow-[0_0_0_3px_var(--accent-soft)]"
            />
            <button
          ...
          <p className="text-xs font-semibold text-[var(--text-dim)] text-center">
          ...
          {isHost && (
            <div className="rounded-xl bg-[var(--bg-elev)] border-2 border-ink shadow-sticker-sm p-3.5 space-y-2">
              ...
              {!user ? (
                <p className="text-xs font-semibold text-[var(--text-dim)] italic">
                ...
                <>
                  <div className="flex gap-2">
                    <input
                      readOnly
                      value={magicLink}
                      className="flex-1 min-w-0 rounded-lg bg-[var(--bg-elev)] border-2 border-ink shadow-sticker-sm px-3.5 py-2 text-xs font-semibold text-[var(--text)] outline-none focus:border-brand-600 focus:shadow-[0_0_0_3px_var(--accent-soft)]"
