...
- **Pen feel tuning**: three layers, all targeting Apple Pencil latency + fountain-pen aesthetic.
  ...
  2. `patches/tldraw+<version>.patch` rewrites `getFreehandOptions` constants in `getPath.{js,mjs}` for both `realPressureSettings` (stylus path, `isPen=true`) and `simulatePressureSettings` (finger/mouse fallback). Targets: `thinning` ~0.8 (strong pressure contrast), `streamline` ~0.4 (less smoothing, more direct), `smoothing` ~0.55, plus `start/end.taper` 25–30 for calligraphic tapered ends. The SAME patch also edits `default-shape-constants.{js,mjs}` to set `STROKE_SIZES.s` to `1.5` (from tldraw's `2`) so the thinnest stroke is finer. Applied via `patch-package --partial` on every `npm install` (postinstall hook). **The `--partial` flag is load-bearing for CI**: Vercel caches `node_modules`, so when the patch FILE changes, a cached build has the OLD edits already applied (e.g. `getPath` patched) but the NEW ones (e.g. `STROKE_SIZES`) not — a mixed state that strict `git apply` can resolve neither forward (old hunks already applied) nor reverse (new hunks absent), hard-failing the build. `--partial` downgrades the already-applied hunks to warnings and still applies the new ones, converging on the correct fully-patched state (verified: on a pristine install it applies everything; on the mixed cache it skips the applied hunks and applies the rest). Trade-off: a GENUINELY broken patch after a tldraw upgrade becomes a warning instead of a hard error, so the pen feel could silently revert — when upgrading tldraw, watch the install output and test drawing locally. When tldraw upgrades: `rm -rf node_modules/tldraw && npm install` — if patch-package warns, re-edit BOTH files (`getPath` pressure settings AND `STROKE_SIZES.s`) in `dist-cjs` + `dist-esm`, then `npx patch-package tldraw` to regenerate.
