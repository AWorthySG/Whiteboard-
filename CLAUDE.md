...
- **Stroke thickness**: default size is `"s"` (small) — set via `editor.setStyleForNextShapes(DefaultSizeStyle, "s")` in `WhiteboardCanvas.onMount`. tldraw's `"m"` was too thick under Apple Pencil pressure. Users can still pick any size from the size picker. The `"s"` width itself is also patched thinner than tldraw's default: `STROKE_SIZES.s` is `1.5` (tldraw ships `2`), edited in `default-shape-constants.{js,mjs}` and captured by the tldraw patch (see pen-feel note). This thins the smallest size for every shape type, but only pen/highlighter are reachable.
...
11. **The tldraw patch survives upgrades only if you re-apply it.** When you bump
    ...
    the fountain-pen values (`thinning: 0.82/0.7`, `streamline: 0` — stabilisation
    is off, see the pen-feel note — `smoothing: 0.55`), and `STROKE_SIZES.s = 1.5`
    in `default-shape-constants.{js,mjs}`,
