...
- `useSyncToken(roomId, userId)` — fetches an HS256 sync token from `/api/sync-token` and auto-refreshes ~2 min before its 15-min TTL. Until the first token arrives, `WhiteboardCanvas` uses a placeholder URI that 401s — useSync briefly shows offline state and swaps to the real URI when the token lands. The route answers only for an ADMITTED `join_requests` row, so the FIRST token is started early with `prefetchSyncToken(roomId, userId)` — by RoomShell's host self-admit once its upsert succeeds (or the row is confirmed admitted), and by `<PrefetchSyncToken>` in the room tree for guests (it renders only once KnockGate admits them) — and the hook adopts that in-flight request instead of waiting for the lazy canvas chunk to mount. Refused/failed fetches retry at 0.3 / 0.8 / 2 s, then every 5 s (`tokenRetryDelay`). Before, a brand-new room (every new lesson) raced the host's self-admit against the canvas's first token request; losing cost a flat 5 s with the board offline. Tests: `useSyncToken.test.tsx`.
...
- `imageCompression.ts` — images are shrunk before they reach the board. `shrinkImageForUpload(file)` scales a photo so its longest side is ≤ `MAX_IMAGE_DIMENSION` (2048 px) and re-encodes it (WebP 0.85; JPEG where the browser can't encode WebP and the image is opaque; PNG to keep transparency), applying EXIF rotation via `createImageBitmap({ imageOrientation: "from-image" })`. Small in-limit images (< 600 KB) and GIFs (may be animated) upload untouched, and a same-size re-encode is kept only if it saves ≥ 10%. It never throws — any failure uploads the original. Called in BOTH canvas image paths: `insertFileOntoCanvas` (paste, file picker) and `makeAssetStore().upload` (images dropped onto tldraw). The two PDF importers encode each rasterised page with `encodeCanvas(canvas, PAGE_QUALITY)` (WebP 0.9; the JPEG fallback checks for transparency first) instead of PNG, and name/type the page file from the resulting blob (`renameForType`, because upload validation checks the extension). Why: a 12 MP phone photo placed at full size is decoded and painted on every pan/zoom (competing with the pen on an iPad) and downloaded in full by every student. The Documents drawer and the host's homework worksheet attachments are NOT shrunk (kept as originals); students' homework submissions ARE, via `AttachmentPicker`'s `shrinkPhotos` prop at a larger 2560 px (`HOMEWORK_PHOTO_MAX`) so handwriting stays legible. Pure decisions are unit-tested in `imageCompression.test.ts`, with a source check that both canvas paths still call it.
- `tidyPage.ts` — "Tidy this page" (host only; the heavy-page pill and the palette command "Tidy this page"). `planTidy` takes the host's own top-level `draw`/`highlight` strokes on the current page (NOT students' `meta.annotation` strokes, ink parented to a post-it, locked or hidden shapes; needs ≥ `TIDY_MIN_STROKES`), groups them into `TIDY_CELL` (1500-unit) cells, and gives each tile a pixel ratio (2×, lower so the bitmap stays ≤ 4096 px) and a z-index just under its lowest stroke. WhiteboardCanvas's `tidyCurrentPage` confirms, renders each tile with `editor.toImage({ bounds, pixelRatio, background: false, format: "webp" })`, uploads it, then `applyTidy` swaps strokes for locked `image` shapes (`meta.uploadedDocument` + `meta.tidiedInk`) in ONE undo step — skipping any tile whose strokes changed during the upload (record identity check) and sweeping those uploads. Tidied ink looks the same at 100% but can't be erased stroke by stroke (Undo, or Delete/Unlock via UploadedItemControls, which doesn't auto-announce `tidiedInk`). Zoomed far out it looks slightly bolder than live strokes, because tldraw draws live strokes thinner at low zoom. Tests: `tidyPage.test.ts` (headless Editor).
- `HeavyPageNotice` (component) — host-only sun-tinted card in CanvasFloatingPanel once the current page holds `HEAVY_PAGE_SHAPES` (3000) shapes: "New page" (same path as PagesTabBar's button, naming dialog included) or "Tidy page". × snoozes it for that page until 2000 more shapes. Otherwise, when the WHOLE room is heavy (`isHeavyRoom`: ≥ 8000 shapes or ≥ 20 pages, re-counted every 20 s — never per change, it walks every record), it offers "Save & start fresh" (RoomShell's `startFresh`, passed down as `onStartFresh`).
- `startFresh.ts` — "Save & start fresh" (host: palette, both More menus, the heavy-room card). Joining a room sends EVERY page, so a room reused lesson after lesson gets slower to open. RoomShell's `startFresh` exports every page with `exportLessonPdf` (Documents + `downloadPdfBlob` to the device) and only if that upload succeeded calls `clearRoomToFreshPage(editor, newPageId)`: one blank "Page 1", one undo step, locked items included (`ignoreShapeLock`), asset records KEPT (deleteAssets isn't undoable — Undo would restore broken pictures).
- `pageCleanup.ts` — **tldraw 3.15's `editor.deletePage` removes only the page record; the page's shapes stay in the store as invisible orphans**, still synced to everyone and saved in the snapshot. Always delete a page with `deletePageWithShapes(editor, id)` (PagesTabBar's × and startFresh do). The host also runs `removeOrphanedShapes` once per room load (3 s after first sync, out of undo history) to sweep up orphans from before the fix. `pageCleanup.test.ts` pins the tldraw behaviour — if a tldraw upgrade makes deletePage clean up after itself, that test will say so.
- `glideCamera.ts` — "Bring everyone to my view" now GLIDES the students' view (`glideToBounds`, ~450 ms, straight-line centre + geometric zoom, cancelled by the student touching the board, a jump under prefers-reduced-motion). tldraw's own `zoomToBounds({ animation })` could not: **tldraw skips every camera animation when `animationSpeed` is 0**, which the app sets so strokes snap in. Any other camera animation you add will be instant for the same reason — tween it like this.
- `roomPreview.ts` — instant reopen. While in a room, this device keeps (IndexedDB `wb-room-preview`, newest 12 rooms) a WebP of the shapes in view plus the current page, camera and the canvas's on-screen rect. It saves after 4 s without changes, at most every 30 s, never mid-stroke (`isWriting()`), when idle, and immediately when the tab is hidden. On the next open WhiteboardCanvas shows that picture in **tldraw's `LoadingScreen` slot** (`PreviewLoadingScreen` via `RoomPreviewContext`; anything layered over the canvas sat UNDER tldraw's loading screen) with the "Connecting…" banner, then on the first sync `restoreView`s the saved page + camera (not for a student following the host) so the live board appears exactly where the picture was, and a fading overlay covers the swap. It never writes to the synced document — a stale picture is only ever a placeholder. Previously every reopen started on the first page at the default camera. The picture is placed with `position: fixed` at the saved `screenX/Y` because tldraw's viewport starts right of the LeftRail, not at the canvas area's edge.
...
- **pdf.js worker** — served from our origin at `/pdfjs/<version>/pdf.worker.min.mjs` (copied from `node_modules/pdfjs-dist` by `scripts/copy-tldraw-assets.mjs`, gitignored, immutable Cache-Control in `next.config.js`, cache-first in `sw.js`). The importers set `workerSrc = pdfWorkerUrl(pdfjs.version)`, so the worker always matches the bundled library — there is no hardcoded version to keep in sync. It used to load ~1.4 MB from cdn.jsdelivr.net on every PDF import. `uploadCacheControl.test.ts` fails if a CDN URL comes back.
...
- **Service worker caching strategy**: `public/sw.js` runs two cache buckets. `wb-static-v4` is cache-first for `/_next/static/*` (content-hashed by Next so safe-to-cache-forever) and `/tldraw-assets/*` (versioned folder) — every PWA cold launch after the first boots from cache, dropping startup ~1s. `wb-shell-v4` is stale-while-revalidate for `manifest.webmanifest` + `icon.svg`. Everything else (HTML routes, API calls, Supabase, LiveKit, sync worker) is network-only — no risk of a stale room shell or stale auth token. If you change the cache schema, bump both bucket names (`-v4` → `-v5`); the `activate` listener sweeps any older bucket.
...
npm test             # vitest run (219 tests across 24 files)
