...
- **Pen mode / palm rejection**: tldraw auto-enables `isPenMode` on first pointerType==='pen' event. We also expose an explicit `penOnly` setting that forces it on at mount. Note: `isPenMode` only gates canvas drawing strokes — toolbar buttons are standard HTML `<button>` elements that respond to any pointer event, so the Apple Pencil can still tap tool buttons to switch tools even in pen-only mode.
- **Apple Pencil Pro squeeze (iPadOS 17.5+) — cannot be disabled**: The squeeze gesture is handled by iPadOS at the OS level, before any event reaches Safari or the app's JavaScript. There is no Web API to suppress it (no `touch-action` equivalent for stylus squeeze). Squeezing mid-stroke fires `pointercancel`, which tldraw handles gracefully by discarding the in-progress stroke. The iPadOS system tool picker may appear as an overlay but doesn't interact with app state. Do not attempt to "fix" this — the platform doesn't provide a hook for web content.
...
- **Zoom UI is custom**. tldraw's `NavigationPanel` (bottom-left "100% >" pill) and `MenuPanel` are both nulled in our `components` override. We render our own `ZoomControls` instead — touch-friendly targets, themed styling, full preset menu, anchored bottom-left so the video panel doesn't cover it.
